...
*Note, this presentation was created by a student (who went on to work at an Investment Bank).  Here is the original file: https://github.com/FinancialMarkets/5MinuteFinance/blob/master/Options/option_straddle/option_straddle.Rmd*
...
What if instead of betting on the direction of the stock's movement, you want to bet on the size of the stock's movement? 
...
>- Since there was no way of knowing which way the vote would go beforehand, the long straddle may have been a good strategy to take advantage of a large market shift in either direction.
...
In the next slide, you'll be able to select a strike price and option premium then view the corresponding payoff diagrams for the long straddle.  For simplicity we'll assume the call and put option have the same premium---so you only have to enter one.  Generally, however, the call and put price will have different premia.
...
>- Since the long straddle benefits from actual volatility being higher than what the market expected, you can potentially earn more (or lose more) if the stock's volatility is greater (volatility of volatility).
